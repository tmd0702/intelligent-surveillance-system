...
import New from "../new-camera";
import Edit from "../edit-camera";
import View from "../view-camera";
...
function CameraList() {
  ...
  useEffect(() => {
    const getCamerasPromise = CameraServices.getCameras();
    const getLocationsPromise = LocationServices.getLocations();
    Promise.all([getCamerasPromise, getLocationsPromise]).then(results => {
      if (results[0]?.success) {
        setCameras(results[0].data);
  ...
  const handleClickAdd = () => {
    setModalPage(<New locations={locations} setList={setCameras} list={cameras} handleClickView={handleClickView}/>)
    setOpenModal(true);
  }
  const handleClickUpdate = (detail) => {
    setModalPage(<Edit info={detail} locations={locations} handleClickView={handleClickView}/>)
    setOpenModal(true);
  }
