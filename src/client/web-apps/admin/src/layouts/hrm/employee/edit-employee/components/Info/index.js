...
function Info({setInfo, info, departments}) {
  ...
  const handleCapture = () => {
    ...
    EmployeeServices.regisFace(info.id, base64Frame).then(result => {
      if (result?.success) {
  ...
  return (
    <MDBox>
      ...
      <MDBox mt={2}>
        <Grid container spacing={3}>
        <Grid item xs={12} sm={6}>
            <FormField type="text" label="Gender" value={info.email} onChange={(e) => setInfo({...info, gender: e.target.value})}/>
          </Grid>
          <Grid item xs={12} sm={6}>
          <MDTypography
            component="label"
          ...
          <Autocomplete
                ...
                value={info.department_id.id? info.department_id : info.department}
